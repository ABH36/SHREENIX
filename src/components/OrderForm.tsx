    ...
    fetch('/api/admin/product')
      ...
      .then(data => {
        if (data.success && data.product) {
          ...
          if (data.product.heroImages && data.product.heroImages.length > 0) setProductImage(data.product.heroImages[0]);
        }
      })
      .catch(() => console.error('Error loading product'));
  }, []);
        ...
        <div className="flex flex-col lg:flex-row gap-10 bg-white rounded-3xl shadow-xl overflow-hidden border border-stone-100">
                ...
                {!appliedCoupon ? (
                  <div className="flex gap-2">
                    <input
                      ...
                      placeholder="CODE"
                      className="w-full bg-emerald-900/50 border border-emerald-600 rounded-lg px-3 py-2 text-sm text-white uppercase outline-none placeholder:text-emerald-400"
                    />
                    <button
                      onClick={handleApplyCoupon}
                      disabled={verifyingCoupon || !couponCode}
                      className="bg-emerald-500 hover:bg-emerald-400 text-emerald-950 font-bold px-3 py-2 rounded-lg text-sm"
                    >
                      Apply
                    </button>
                  </div>
                  ...
                  <div className="flex justify-between items-center bg-emerald-500/20 p-2 rounded-lg border border-emerald-500/30">
                    ...
                    <button onClick={removeCoupon}>
                      <X size={16} className="text-emerald-200 hover:text-white" />
                    </button>
                  </div>
                )}
                {couponMsg && (
          ...
          <div className="lg:w-2/3 p-8 lg:p-12 relative">
            <AnimatePresence>
              {success && (
                ...
                >
                  ...
                  <button onClick={() => setSuccess(false)} className="mt-8 text-emerald-600 font-bold underline">
                    Place Another Order
                  </button>
                </motion.div>
              )}
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              {errorMsg && (
                <div className="text-red-600 bg-red-50 p-3 rounded-lg text-sm font-bold flex gap-2">
              ...
              <button
                ...
                className="w-full bg-emerald-800 hover:bg-black text-white font-bold py-4 rounded-xl shadow-lg flex items-center justify-center gap-2 text-lg mt-4 disabled:opacity-70 disabled:cursor-not-allowed transition-colors"
              >
